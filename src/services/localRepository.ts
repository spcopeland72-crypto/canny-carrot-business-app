    ...
    if (businessResponse.ok) {
      ...
      if (businessResult.success && businessResult.data) {
        ...
        const profile: BusinessProfile = {
          ...
          addressLine1: businessData.address?.line1 || businessData.profile?.addressLine1 || (businessData as { addressLine1?: string }).addressLine1 || '',
          addressLine2: businessData.address?.line2 || businessData.profile?.addressLine2 || (businessData as { addressLine2?: string }).addressLine2 || '',
          city: businessData.address?.city || businessData.profile?.city || (businessData as { city?: string }).city || '',
          postcode: businessData.address?.postcode || businessData.profile?.postcode || (businessData as { postcode?: string }).postcode || '',
          region: businessData.profile?.region || (businessData as { region?: string }).region,
          country: businessData.profile?.country || (businessData as { country?: string }).country || 'UK',
          logo: businessData.logo || businessData.profile?.logo || (businessData as { logo?: string }).logo,
          logoIcon: businessData.profile?.logoIcon ?? (businessData as { logoIcon?: string }).logoIcon,
          banner: businessData.profile?.banner ?? (businessData as { banner?: string }).banner,
